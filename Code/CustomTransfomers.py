...
class SlidingWindow(BaseEstimator, TransformerMixin):
    ...
    '''
    def __init__(self, channelNames, samplingRate, windowSizeInSeconds):
...
class ExtractSignals(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df : pd.DataFrame):
        ...
        if self.device == DEVICES_MUSE_MONITOR:
            ...
            new_df = pd.DataFrame()
            new_df['TP9'] = df['RAW_TP9'].copy()
            new_df['AF7'] = df['RAW_AF7'].copy()
            new_df['AF8'] = df['RAW_AF8'].copy()
            new_df['TP10'] = df['RAW_TP10'].copy()
